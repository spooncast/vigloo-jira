...
- `s`: 서브태스크 상태 변경 (해야 할 일 / 진행 중 / 검토 중 / 완료). 팝업에서 `↑↓`로 선택 후 `Enter`, `Esc`로 취소. 화면에 즉시 반영되고 변경은 백그라운드에서 처리
...
vj write           # 내일 스크럼 코멘트 자동 작성 (오늘 할 것 → 내일 한 것)
vj write today     # 오늘 스크럼 코멘트 자동 작성 (어제 할 것 → 오늘 한 것)
...
| `Esc` | 뒤로 |
| `Tab` | 패널 전환 (Sprint 모드) |
| `s` | 서브태스크 상태 변경 (Sprint 모드, 서브태스크 패널) |
